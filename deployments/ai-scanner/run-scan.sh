...
case "$KIND" in
  post-market)
    PROMPT="Выполни пост-маркет анализ за $DATE по инструкции из system-prompt.md. Сначала прочитай справочник $REF и память $MEM. Затем: 1) Сканируй ТОЛЬКО фьючерсы с помощью scan_market.py, учитывай contract_price <= AI_SCANNER_MAX_CONTRACT_PRICE 2) Не отсекай кандидатов по score/atr_pct до бэктеста — score только ранжирует очередь 3) Для каждого futures-кандидата из результата сканера запусти бэктест sma и level_bounce с --optimize 4) Отбери только стратегии с Sharpe>1, PnL>0, win_rate>45%, trades>=5, profit_factor>1.3 5) Обнови config.yaml — добавляй только SPBFUT futures стратегии, убери/замени убыточные auto-стратегии 6) Вызови POST /config/reload на strategy-runner 7) Если config.yaml менялся, подожди scrape и запусти: python3 /opt/ai-scanner/monitoring/strategy_dashboard_smoke.py --strategy-runner-url $STRATEGY_RUNNER_URL --dashboard-json /workspace/reference/24alert-strategy-runner.json; без зелёного smoke-check нельзя считать работу завершённой 8) Запиши отчёт в $REPORTS, включая результат smoke-check 9) Дополни $MEM краткой записью о решениях и метриках. Действуй автономно, без подтверждений."
    ;;
  pre-market)
    PROMPT="Выполни пре-маркет проверку за $DATE по инструкции из system-prompt.md. Сначала прочитай справочник $REF, skill логов $LOG_SKILL и память $MEM. Затем: 1) Проверь здоровье всех сервисов (gateway, strategy-runner) 2) Проверь что все enabled стратегии запущены через GET /instances 3) Запусти: python3 /opt/ai-scanner/monitoring/strategy_dashboard_smoke.py --strategy-runner-url $STRATEGY_RUNNER_URL --dashboard-json /workspace/reference/24alert-strategy-runner.json и убедись что strategy tiles/dashboard contract не сломан 4) Проверь PnL каждой стратегии за вчера 5) Проверь journal events по каждой стратегии через /instances/<id>/events?limit=50 6) Запиши краткий отчёт в $REPORTS 7) При значимых событиях дополни $MEM. Действуй автономно."
    ;;
  health)
    PROMPT="Быстрая проверка здоровья 24alert за $DATE: сначала прочитай $REF, $LOG_SKILL и $MEM, затем 1) GET /health на gateway и strategy-runner 2) GET /instances — убедись что все enabled стратегии running 3) Запусти: python3 /opt/ai-scanner/monitoring/strategy_dashboard_smoke.py --strategy-runner-url $STRATEGY_RUNNER_URL --dashboard-json /workspace/reference/24alert-strategy-runner.json 4) Проверь journal events через /instances/<id>/events?limit=20 5) Если что-то не так — запиши в $REPORTS, попробуй перезапустить через API и дополни $MEM. Кратко."
    ;;
  *)
