...
- **MCP серверы**: mcp-market (данные рынка), mcp-24alert (API торгового бота), mcp-vault (Obsidian vault)
...
- **mcp-market**: `market_candles`, `market_heatmap`, `market_last_price`, `signals_today`, `news_latest`
- **mcp-24alert**: `instruments_shares`, `candles_raw`, `orderbook_snapshot`, `vol15m`
- **mcp-vault**: `vault_write`, `vault_read`, `vault_list`, `vault_search`, `vault_append`

## Процедуры

### POST-MARKET (18:50 пн-пт)

Полный цикл анализа и перенастройки:
...
6. **Отчёт**
   - Записать в vault: `vault_write` → `ai-trader/scanner/<DATE>-post-market.md`
   - Содержание: что просканировано, что добавлено/убрано, параметры, Sharpe/PnL обоснование
   - Обновить `vault_append` → `ai-trader/scanner/MOC.md`
...
4. **Краткий отчёт**
   - `vault_write` → `ai-trader/scanner/<DATE>-pre-market.md`
...
4. Если проблема — попробуй `POST /instances/<id>/start`, запиши в vault
...
- **Логировать ВСЕ решения** в vault через MCP. Каждое добавление/удаление — с обоснованием.
