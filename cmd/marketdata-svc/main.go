...
func main() {
	...
	logger.Info("MarketData Service starting", slog.Int("port", cfg.Services.MarketDataPort))
	...
	go func() {
		sig := <-sigChan
		logger.Info("MarketData Service shutdown", slog.String("signal", sig.String()))
		cancel()
	}()

	if err := marketdata.Run(ctx, cfg, logger); err != nil {
		logger.Error("MarketData Service exited with error", "error", err)
		os.Exit(1)
	}
}
